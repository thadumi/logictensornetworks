...
y_ = tf.compat.v1.placeholder(tf.float32, shape=[None, embedding_size*2],name="y_")
...
correct_prediction = tf.equal(
    tf.argmax(input=tf.reshape(y_conv,(-1,embedding_size)), axis=1),
    tf.argmax(input=tf.reshape(y_,(-1,embedding_size)), axis=1))
...
examples_of_1 = {n:ltn.variable("examples_of_1_"+str(n),
                    tf.gather(y_conv[:train_batch_size],
                              tf.compat.v1.where(tf.equal(y_[:train_batch_size,tf.constant(n)],1))[:,0]))
                 for n in range(10)}

examples_of_2 = {n:ltn.variable("examples_of_2_"+str(n),
                    tf.gather(y_conv[:train_batch_size],
                              tf.compat.v1.where(tf.equal(y_[:train_batch_size,tf.constant(10+n)],1))[:,0]))
                 for n in range(10)}
...
constr_min = tf.reduce_min(input_tensor=constraints,axis=0)
constr_hmean = 1/tf.reduce_mean(input_tensor=1/constraints)
constr_mean = tf.reduce_mean(input_tensor=constraints,axis=0)
constr_sqmean = tf.reduce_mean(input_tensor=tf.square(constraints),axis=0)
constr_var = tf.reduce_mean(input_tensor=tf.square(constraints-constr_mean))
constraints_loss = (1.0-constr_mean)

optimize_only_conv_net = tf.compat.v1.train.AdamOptimizer(1e-3).minimize(cross_entropy)
optimize_conv_net_and_ltn = tf.compat.v1.train.AdamOptimizer(1e-3).minimize(cross_entropy+constraints_loss)
optimize_only_ltn = tf.compat.v1.train.AdamOptimizer(1e-3).minimize(constraints_loss)
