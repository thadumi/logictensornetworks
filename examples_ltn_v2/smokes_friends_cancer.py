...
with G.as_default():
    ...
    Friends(g[friends[0][0]], g[friends[0][1]])
    ...
    loss = 1.0 / tf.reduce_mean(input_tensor=1 / tf.concat(facts, axis=0))
    opt = tf.compat.v1.train.RMSPropOptimizer(learning_rate=.01, decay=.9)
    optimize = opt.minimize(-(loss + ltn.BIAS))

    with tf.compat.v1.Session(graph=G) as sess:
        # writer = tf.summary.create_file_writer('/tmp/tf/logs')
        # all_summary_ops = tf.compat.v1.summary.all_v2_summary_ops()

        # sess.run(writer.init())

        sess.run(tf.compat.v1.global_variables_initializer())
        writer = tf.compat.v1.summary.FileWriter('/tmp/tf/logs', sess.graph)
