...
def initialize_tf_variables(all=False):
    if tf.compat.v1.global_variables():
        if all:
            ...
            uninitialized_variable_names=set([v.decode("ascii") for v in SESSION.run(tf.compat.v1.report_uninitialized_variables())])
            uninitialized_variables=[var for var in tf.compat.v1.global_variables() if var.name.split(":")[0] in uninitialized_variable_names]
            init=tf.compat.v1.variables_initializer(uninitialized_variables)
        SESSION.run(init)
...
def initialize_knowledgebase(optimizer=None,
    formula_aggregator=lambda *x: tf.reduce_mean(input_tensor=tf.concat(x,axis=0)) if x else None,
    ...
    if tf.compat.v1.global_variables() and KNOWLEDGEBASE is not None:
        ...
        OPTIMIZER = optimizer or tf.compat.v1.train.GradientDescentOptimizer(learning_rate=0.1)
